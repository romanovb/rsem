...
# Задание 15 Gapminder data

```{r}
library(readxl)
library(stringr)
hiv <- read_xlsx(path = 'hiv.xlsx', sheet = "frame")
head(hiv)
names(hiv) <- str_to_lower(names(hiv))
hiv <- as.data.frame(sapply(hiv, as.numeric))
hivc <- filter(hiv, country > 1989)
ggplot(data=hivc) + geom_freqpoly(mapping = aes(x= bahamas, y = ..count.., color = "bahamas")) + geom_freqpoly(mapping = aes(x = barbados, y = ..count.., color = "barbados")) + labs(x = "Значение показателя", color = "Страна")

```


На Багамских островах процент зараженных HIV выше, чем в Барбадосе.

# Задание 16 - Friends Birthdays
